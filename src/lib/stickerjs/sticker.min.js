...
(function () {
    ...
    function checkDerection(e, pos, sizeQ) {
        ...
        if (tx < sizeQ)direction = 0; else if (tx > sizeQ * 3)direction = 1; else if (ty < sizeQ)direction = 2; else direction = 3;
        return direction
    }

    function checkPos(e, pos, size) {
        var fx = pos.x, fy = pos.y, tx = e.pageX - fx, ty = e.pageY - fy, value, a = size - tx, b = size - ty, c = tx >> 1, d = ty >> 1, e = a >> 1, f = b >> 1;
    ...
    function onEnter(e, value) {
        var cpos = value.container.getBoundingClientRect(), mpos = {
        ...
        css(value.move, {transition: _setTrans, transform: "translate(" + 0 + "px, " + 0 + "px)"});
        css(value.back, {transition: _setTrans, transform: "translate(" + bx + "px, " + by + "px)"});
        css(value.backImg, {transform: "scaleX(" + sx + ") scaleY(" + sy + ")"});
        css(value.depth, {transform: "translate(" + -1e4 + "px, " + -1e4 + "px)"})
    }
    ...
    function onLeave(e, value) {
        ...
        css(value.move, {transition: _aniTrans, transform: "translate(" + 0 + "px, " + 0 + "px)"});
    ...
    function onMove(e, value) {
        ...
        var pos = checkPos(e,
            _savePos.pos,
            value.size), bmx = pos.bmx, bmy = pos.bmy, bsw = pos.bsw, bsh = pos.bsh, bsx = pos.bsx, bsy = pos.bsy, cw = pos.cw, ch = pos.ch, cx = pos.cx, cy = pos.cy, dw = pos.dw, dh = pos.dh, dx = pos.dx, dy = pos.dy;
        css(value.mask, {width: cw + "px", height: ch + "px", transform: "translate(" + cx + "px, " + cy + "px)"});
        css(value.move, {transform: "translate(" + -cx + "px, " + -cy + "px)"});
        css(value.back, {transform: "translate(" + bmx + "px, " + bmy + "px)"});
        ...
        css(value.depth, {width: dw + "px", height: dh + "px", transform: "translate(" + dx + "px, " + dy + "px)"})
    }
    ...
    var sticker = {
        init: function init(dom) {
            ...
            var value, pos = dom.getBoundingClientRect(), size = pos.width, sizeQ = size >> 2, container = createEl("div",
                {
                    position: "relative",
                    width: size + "px",
                    height: size + "px",
    ...
    if (typeof exports == "object")module.exports = sticker; else if (typeof define == "function" && define.amd)define(function () {
        return sticker
